...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="record">
        ...
        <mods:originInfo>
            <mods:dateCaptured format="{date/@format}">
    ...
    <xsl:template match="mediafile">
        <mods:typeOfResource collection="no" manuscript="no">
            <xsl:choose>
                <xsl:when test="clip_format/text() = 'video'">
                    moving image
                </xsl:when>
                <xsl:otherwise>
                    sound recording
    ...
    <xsl:template match="interviewee | interviewer">
        <mods:name type="personal">
            ...
            <mods:role>
                <mods:roleTerm type="text"><xsl:value-of select="local-name()"/></mods:roleTerm>
            </mods:role>
        </mods:name>
    </xsl:template>
    ...
    <xsl:template match="index">
        <mods:relatedItem type="constituent">
            <xsl:apply-templates select="point/title" />
        </mods:relatedItem>
    </xsl:template>
    
    <xsl:template match="point/title">
